...
let s:candidates = v:lua.require('vimtex.test').completion('\', 'test')
...
let s:candidates = v:lua.require('vimtex.test').completion('\', 'DeclareD')
...
let s:candidates = v:lua.require('vimtex.test').completion('\', 'glsentry.*acc')
call assert_true(len(s:candidates) > 0)
