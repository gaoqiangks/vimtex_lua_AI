...
" Local caches
let s:cache1 = v:lua.require('vimtex.cache').open('test-local', {'local': 1})
...
let s:cache2 = v:lua.require('vimtex.cache').open('test-local', {'local': 1})
call assert_equal('./test-local%foo.json', s:cache2.path)
call s:cache2.set('a', 2)

" The caches are not the same!
call v:lua.require('vimtex.cache').write_all()
