...
function! TestBuildDir(expected) abort
  ...
  " let l:out_dir = v:lua.require('vimtex.paths').shorten_relative(
...
call TestBuildDir('out')
call assert_true(empty(v:lua.require('vimtex.log').get()))

let $VIMTEX_OUTPUT_DIRECTORY = 'build'
call TestBuildDir('build')
let s:warning = get(map(v:lua.require('vimtex.log').get(), {_, x -> x.msg[0]}), 0, 'NONE')
call assert_equal(
