...
for [s:in, s:out] in [
      ...
      \]
  if s:in =~# '\/'
    call v:lua.require('vimtex.test').keys('f/ltsf', s:in, s:out)
    call v:lua.require('vimtex.test').keys('f/htsf', s:in, s:out)
  else
    call v:lua.require('vimtex.test').keys('f{tsf', s:in, s:out)
  endif
endfor

" tsf  /  Toggle surrounding fraction (visual mode)
call v:lua.require('vimtex.test').keys('f$lvf$htsf',
      \ 'testing $inline frac / something$ more text',
      \ 'testing $\frac{inline frac}{something}$ more text')
call v:lua.require('vimtex.test').keys('f/bvf$htsf',
      \ 'testing $inline frac / something$ more text',
      \ 'testing $inline \frac{frac}{something}$ more text')
call v:lua.require('vimtex.test').keys('f/bvtttsf',
      \ 'testing $inline frac / something$ more text',
      \ 'testing $inline \frac{frac}{some}thing$ more text')
call v:lua.require('vimtex.test').keys('f(v$hhtsf',
      \ '$(\delta_{02})/(\delta_{02} + \delta_{01})$',
      \ '$\frac{\delta_{02}}{\delta_{02} + \delta_{01}}$')
call v:lua.require('vimtex.test').keys('f\v$hhtsf',
      \ '$\frac{\delta_{02}}{\delta_{02} + \delta_{01}}$',
