...
silent call v:lua.require('vimtex.test').completion('\cite{', '')
echo 'Backend:' toupper(g:vimtex_parser_bib_backend)
let s:time = v:lua.require('vimtex.profile').time(s:time, 'Time elapsed (1st run)')

" profile pause

call v:lua.require('vimtex.test').completion('\cite{', '')
let s:time = v:lua.require('vimtex.profile').time(s:time, 'Time elapsed (2nd run)')

let s:candidates = v:lua.require('vimtex.test').completion('\cite{', '')
let s:time = v:lua.require('vimtex.profile').time(s:time, 'Time elapsed (3rd run)')
